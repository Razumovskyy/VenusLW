...
CHARACTER*260 FILNAME
CHARACTER CHANNEL_NUMBER0*3,FI*20,BAN*3,CHANNEL_NUMBER*2 
PARAMETER (JZM=101,N_ATM=1) ! Number of the atmospheric models
CHARACTER*80 ATM_PROF(N_ATM),LBL(N_ATM),K_FILE(N_ATM)
DIMENSION D_TAU(JZM),C_K(JZM),FDO_LBL(JZM),FUP_LBL(JZM),Q_LBL(JZM),FDO_KDS(JZM),FUP_KDS(JZM),Q_KDS(JZM)
COMMON /ZPT/Z(JZM),P(JZM),T(JZM),RO(JZM)
COMMON/PLAN/PL(JZM)
COMMON/CHNN/CHANEL_NUMBER
...
! * -------------------------------------------------------- *
!VVV                           DO N_GAME=1,1  ! 00000
             weight=1.0
! *          READ(*,*) WEIGHT  ! Main fitting parameter "WEIGHT"
            IF(WEIGHT.LT.0.)STOP

             DO N_A=1,N_ATM
!        Atm_pr=FI ! ### ATM_PROF(N_A)
...
!*   'LAYER'      Derivatives                 *
        ...
        Q_LBL(J)=-(XEJ-XEJM)/(P(J)-P(J-1))*8.442/1013.25
           END DO
          Q_LBL(1)=Q_LBL(2)
...
! ********************** CORRECTION of D_TAU for Q improving ***********************************
            ...
            JZM_1=JZM-1
              DO J=JZM_C,J_COR,-1
      	D_TAU(J)=D_TAU(J) -(Q_LBL(J+1)-Q_KDS(J+1))/8.442*(P(J+1)-P(J))*2./(PL(J)+PL(J+1))/1013.25
      	  END DO
              CALL FROMTAUTOFLUX(WEIGHT,FDO_KDS,FUP_KDS,Q_KDS,D_TAU,fup_lbl) ! "Fup,Fdo,Q" from "TAU"
      	END DO
!C	  DO J=JZM_1,1,-1
!C	  PRINT *,'LBL KDS',J,Q_LBL(J),Q_KDS(J)
!C	  PAUSE 6
!C	  END DO
!C	  STOP                 
!**********************************************************************************************
...
COMMON /ZPT/Z(JZM),P(JZM),T(JZM),RO(JZM)
DIMENSION C(JZM),R0(32)  &
,D_TAU(JZM),FDO(JZM),FDO_LBL(JZM),FUP_LBL(JZM),Q_LBL(JZM),TAU(JZM)
!save C,R0,FDO,TAU
DATA OLD_NEW/'OOOOOOOOOOOOOOOOOO'/
!* ------ Z,R,T,RO reading from atmospheric profile ------ * 
               IF(OLD_NEW.NE.ATM_PROF)THEN 
      OLD_NEW=ATM_PROF
OPEN(55,FILE=ATM_PROF)  ! Atmospheric profile  !VVV        OPEN(55,FILE=PTHN//ATM_PROF)  ! Atmospheric profile 
           READ(55,'(A)')TITLE
      READ(55,*)NGAS,JZ
      IF(JZ.GT.JZM) WRITE(*,*)' Number of levels',JZ,'>',JZM,' in DIMENSION'
      IF(JZ.GT.JZM) STOP
         DO I=1,NGAS
         READ(55,'(A)')GAS
         END DO
...
!*******  Iterations ***********************************
	...
	F1=FLUX(WES,TAU,T,J,DT1,JZM)
	F2=FLUX(WES,TAU,T,J,DT2,JZM)
	      FLU=FDO(J)
...
1     DT = (DT1+DT2)*0.5D0
	...
	goto 2
      ...
      write(*,*) "coefff--pass", coefficient, ro(j), ro(j+1), tau(j+1)
      dt=coefficient*(ro(j)+ro(j+1))*0.5+tau(j+1)
 2    D_TAU(J)=DT-TAU(J+1)
	TAU(J)=DT
!      write(*,*) "lasst", "J=", J, "TAU(J)", TAU(J)
      END DO
      !pause 3
! ******* end of the iterations, Writing  **********************
 
 !#### Test: Donward fluxes ### !  26 June, 2022 
 !#### Test:OPEN(629,FILE='LBL-KD.D')
 !#### Test:WRITE(629,*)' 2  100 '
 !%%	DO J=JZMM,1,-1
!%%		Fdow=FLUX(WES,TAU,T,J,TAU(J),JZM)
 !###   write(*,*)j,z(j),fdo(j),Fdow  ;  pause 7
!%% WRITE(629,628)Z(J),fdo(j),Fdow
!%% 628 FORMAT(F7.1,2E12.4)
!%%	end do

!#### Test: 	DO J=1,JZMM
!#### Test:		Fdow=FLUX(WES,TAU,T,J,TAU(J),JZM)
 !  write(*,*)j,z(j),fdo(j),Fdow  ;  pause 7
 !#### Test:WRITE(629,628)Z(J),fdo(j),Fdow
 !#### Test:628 FORMAT(F7.1,2E12.4)
!#### Test:	end do
!#### Test:	CLOSE(629)
!#### Test: 	result=RUNQQ('ToDraw.exe','LBL-KD.D')
END
! ****************************************************************
FUNCTION FLUX(WES,TAUMA,T,JJ,DT,JZM)  ! <--- DOWNWARD Fluxes
! ***  DT=TAUMA(JJ)-TAUMA(JJ+1) *** 
IMPLICIT REAL*8 (A-H,O-Z)
PARAMETER(TLIM=25.,DELT_CRT=0.001,NANGLE=3)
DIMENSION WEIGHT(NANGLE),COSINE(NANGLE),TAUMA(JZM),T(JZM)
