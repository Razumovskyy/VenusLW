SUBROUTINE FROMTAUTOFLUX(NTERM,FLUXDO,FLUXUP,QT,D_TAU,QLBL)
!*** LONG-WAVE calculations by "K-Distribution technique" ***
      IMPLICIT REAL*8 (A-H,O-Z)
      CHARACTER NAME*10, AAA*80
      PARAMETER (JZMD=201,NANGLE=3,TLIM=25.,DELT_CRT=0.001) ! NANGLE=5
      DIMENSION FLUXUP(JZM),FLUXDO(JZM),QT(JZM),XEF(JZM),QLBL(JZM) &
...
!****** Downward fluxes
		...
		ALPHA=(BB1*TATA2-BB2*TATA1)/DELT_TAU
         ...
         TTT1=TTT1+EMIS*WEIGHT(IAN)
           ...
           END IF
!*                   
      FLUXDO(I)=FLUXDO(I)+TTT1   !### *WES *WES
	   END DO 
         	END IF
...
!*   'LAYER'      Derivatives                 *
         DO J=2,JZM
      QT(J)=-(XEF(J)-XEF(J-1))/(P(J)-P(J-1))*CP_CONST
         END DO
        QT(1)=QT(2)
        END            
