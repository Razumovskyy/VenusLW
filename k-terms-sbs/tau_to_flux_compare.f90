...
!*** LONG-WAVE calculations by "K-Distribution technique" ***
      ...
      PARAMETER (JDI=101,NANGLE=3,TLIM=25.,DELT_CRT=0.001) ! 5
      DIMENSION FLUXUP(JDI),FLUXDO(JDI),QT(JDI),XEF(JDI),fluxuplbl(JDI) &
     ,WEIGHT(NANGLE),COSINE(NANGLE),TAUMA(JDI),D_TAU(JDI)
...
!*   'LAYER'      Derivatives                 *
         DO J=2,JZM
      QT(J)=-(XEF(J)-XEF(J-1))/(P(J)-P(J-1))*3.963/1013.25
         END DO
        QT(1)=QT(2)
        END            
